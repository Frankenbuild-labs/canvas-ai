...
import { PegasusAction, usePegasusContext } from "@/components/iiagent/pegasus-context";
import { AgentEvent, TOOL, ActionStep, Message, TAB, WebSocketConnectionState } from "@/lib/agent-types";
import { Terminal as XTerm } from "@xterm/xterm";

export function usePegasusEvents({
  xtermRef,
}: {
    ...
    (
      ...
      switch (data.type) {
        case AgentEvent.AGENT_INITIALIZED: {
          ...
          // Prefer an explicitly configured embedded VS Code URL, fallback to backend-provided URL
          const EMBEDDED_VSCODE =
            (process.env.NEXT_PUBLIC_VSCODE_URL as string | undefined) ||
            (data.content.vscode_url as string | undefined) ||
            "";

          // If workspace is already known, try to pass it to VS Code as a folder parameter
          const workspace = (data.content.workspace_path as string | undefined) || state.workspaceInfo || "";
          const vscodeWithFolder = EMBEDDED_VSCODE
            ? `${EMBEDDED_VSCODE}${EMBEDDED_VSCODE.includes("?") ? "&" : "?"}folder=${encodeURIComponent(
                workspace
              )}`
            : "";
          ...
          safeDispatch({
            type: "SET_VSCODE_URL",
            payload: workspace ? vscodeWithFolder : EMBEDDED_VSCODE,
          });
          break;
        }
        ...
        case AgentEvent.USER_MESSAGE:
          {
            const incoming = (data.content.text as string) || ""
            const last = messagesRef.current[messagesRef.current.length - 1]
            // De-dup: if the last message is the same user text we just added locally, ignore echo
            if (last && last.type === "user" && last.content === incoming) {
              break
            }
            safeDispatch({
              type: "ADD_MESSAGE",
        ...
        case AgentEvent.WORKSPACE_INFO: {
          const path = data.content.path as string;
          safeDispatch({
            type: "SET_WORKSPACE_INFO",
            payload: path,
          });
          // If we have an embedded VS Code base URL configured but vscodeUrl is empty or mismatched,
          // update it to include the current workspace folder.
          const EMBEDDED_VSCODE = (process.env.NEXT_PUBLIC_VSCODE_URL as string | undefined) || "";
          if (EMBEDDED_VSCODE) {
            const vscodeWithFolder = `${EMBEDDED_VSCODE}${EMBEDDED_VSCODE.includes("?") ? "&" : "?"}folder=${encodeURIComponent(
              path
            )}`;
            safeDispatch({ type: "SET_VSCODE_URL", payload: vscodeWithFolder });
          }
          break;
        }
          ...
          if (data.content.tool_name === TOOL.SEQUENTIAL_THINKING) {
            ...
            const message: Message = {
              ...
              action: {
                ...
                tool: data.content.tool_name as TOOL,
                data: data.content,
              },
              timestamp: Date.now(),
            };
            ...
            // If agent indicates returning control via tool, stop loading
            if (
              (data.content.tool_name as string)?.toLowerCase() ===
        ...
        case AgentEvent.FILE_EDIT:
          ...
          if (
            ...
            // Ensure action.data exists and use a local reference for TS narrowing
            const actionData = (lastMessage.action.data ??= {} as any);
            actionData.content = data.content.content as string;
            actionData.path = data.content.path as string;
            ...
            if (
              ...
              if (lastToolCallMessageIndex !== -1) {
                ...
                if (lastToolCallMessage?.action) {
                    const actionData = (lastToolCallMessage.action.data ??= {} as any);
                    actionData.result = `${data.content.result}`;
                  if (
                    [
                      TOOL.BROWSER_VIEW,
                      TOOL.BROWSER_CLICK,
                ...
                safeDispatch({
                  ...
                  payload: {
                    ...
                    action: {
                      type: data.content.tool_name as TOOL,
                      tool: data.content.tool_name as TOOL,
                      data: data.content as any,
                    } as ActionStep,
                  },
                });
              }
              // Stop loading when tool returns a result that ends the action
              if (
                [TOOL.RETURN_CONTROL_TO_USER, TOOL.COMPLETE].includes(
