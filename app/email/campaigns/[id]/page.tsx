...
import {
  ...
  getEmailTemplates,
  getEmailLists,
} from "@/lib/email-marketing/database";
import CampaignPageClient from "@/components/email-marketing/CampaignPageClient";
  ...
  try {
    ...
    if (campaign.metadata.status?.value === "Sent") {
      try {
        ...
        if (hasCosmic) {
          const { getUnsubscribedContactsByCampaign } = await import("@/lib/email-marketing/cosmic");
          const result = await getUnsubscribedContactsByCampaign(id, { limit: 50 });
