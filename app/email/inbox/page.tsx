...
export default function InboxPage() {
  ...
  async function refresh() {
    const url = new URL('/api/email/inbox', window.location.origin)
        ...
        <div className="space-y-2">
          ...
          {messages.map(m => (
            <div key={m.id} className="rounded border bg-card p-3">
              ...
              <div className="text-xs text-muted-foreground truncate">From: {m.from} • To: {m.to} {m.attachments?.length ? `• ${m.attachments.length} attachment${m.attachments.length>1?'s':''}` : ''}</div>
              {m.text ? (
