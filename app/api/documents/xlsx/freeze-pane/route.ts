...
export async function POST(req: NextRequest) {
  try {
    ...
    const rows: number = Number(body?.rows || 1)
    const columns: number = Number(body?.columns || 0)

    let { wb } = await loadLatestWorkbook()
    if (!wb) wb = new ExcelJS.Workbook()
    let ws = wb.getWorksheet(sheetName) || wb.addWorksheet(sheetName)

    ws.views = [{ state: 'frozen', xSplit: columns, ySplit: rows }]

    const { url } = await saveWorkbook(wb, `freeze-pane-${sheetName}`)
