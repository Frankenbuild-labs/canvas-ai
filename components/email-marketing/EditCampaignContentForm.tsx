...
import { MarketingCampaign, TemplateType, EmailTemplate } from "@/lib/email-marketing/types";
import {
  AlertCircle,
  CheckCircle,
  Info,
...
interface EditCampaignContentFormProps {
  ...
  templates?: EmailTemplate[]; // Optional list of templates for applying to campaign
}
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            ...
            <label htmlFor="template-active" className="sr-only">Active Template</label>
            <input
              id="template-active"
              type="checkbox"
...
export default function EditCampaignContentForm({
  ...
  const handleCreateTemplate = async (templateData: {
    ...
    try {
      ...
      setTimeout(() => {
  router.push(`/email/templates/${result.template.id}/edit`);
      }, 1000);
  ...
  const handleAIEdit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/email-marketing/campaigns/edit-ai", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const activeTemplates = templates.filter(t => t.metadata.active);
  const filteredTemplates = activeTemplates.filter(t => {
    ...
    return (
      ...
      (t.metadata.subject || '').toLowerCase().includes(term)
    );
  });
  ...
  return (
    ...
    >
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* Apply Template (uses existing templates) */}
              {!editingSessionActive && canEdit && activeTemplates.length > 0 && (
                <Button
                      ...
                      {isAIEditing && aiStatus && (
                        <div className="p-3 bg-purple-100 border border-purple-200 rounded-lg">
                          ...
                          <div className="w-full bg-purple-200 rounded-full h-2">
                            {(() => {
                              // Map aiProgress to coarse width classes to avoid inline styles
                              const pct = aiProgress;
                              let widthClass = 'w-[2%]';
                              if (pct >= 100) widthClass = 'w-full';
                              else if (pct >= 90) widthClass = 'w-[90%]';
                              else if (pct >= 80) widthClass = 'w-[80%]';
                              else if (pct >= 70) widthClass = 'w-[70%]';
                              else if (pct >= 60) widthClass = 'w-[60%]';
                              else if (pct >= 50) widthClass = 'w-1/2';
                              else if (pct >= 40) widthClass = 'w-[40%]';
                              else if (pct >= 30) widthClass = 'w-[30%]';
                              else if (pct >= 20) widthClass = 'w-[20%]';
                              else if (pct >= 10) widthClass = 'w-[10%]';
                              return <div className={`bg-purple-600 h-2 rounded-full transition-all duration-300 ${widthClass}`}></div>;
                            })()}
                          </div>
                        </div>
                      )}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-lg overflow-hidden">
                  ...
                  <div
                    className="p-4 max-h-96 overflow-y-auto bg-white font-sans leading-relaxed"
      ...
      {showApplyTemplateDialog && (
        <Dialog open={true} onOpenChange={(open) => !open && setShowApplyTemplateDialog(false)}>
          <DialogContent className="sm:max-w-[700px]">
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="template-search" className="text-sm font-medium">Search Templates</Label>
                <span className="text-xs text-gray-500">{filteredTemplates.length} shown</span>
              </div>
              <Input
                id="template-search"
              ...
              <div className="max-h-[360px] overflow-y-auto space-y-2 pr-1">
                {filteredTemplates.map(t => (
                  <div key={t.id} className="border rounded-md p-3 bg-white hover:bg-green-50 transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="font-medium text-gray-900">{t.metadata.name}</div>
                        <div className="text-xs text-gray-500 mt-0.5 line-clamp-2">{t.metadata.subject}</div>
                        {t.metadata.preview_html && (
                          <div
                            className="mt-2 text-xs text-gray-600 line-clamp-3"
                            // eslint-disable-next-line react/no-danger
                            dangerouslySetInnerHTML={{ __html: t.metadata.preview_html }}
                          />
                        )}
                      </div>
                      <Button
                        ...
                        onClick={() => {
                          ...
                          try {
                            // Start editing session if not active yet
                            if (!editingSessionActive) {
                              startEditingSession();
                            }
                            // Apply template content to form (NOT display data until saved)
                            setFormData(prev => ({
                              ...prev,
                              subject: t.metadata.subject || prev.subject,
                              content: t.metadata.content || prev.content,
                              template_type: (t.metadata.template_type?.value as TemplateType) || prev.template_type,
                            }));
                            toast({
                              title: 'Template Applied',
                              description: `Loaded "${t.metadata.name}" into editor. Remember to Save to persist.`,
                              variant: 'success'
                            });
                ...
                {filteredTemplates.length === 0 && (
                  <div className="p-6 text-center text-sm text-gray-500">No templates match your search.</div>
                )}
              </div>
              <div className="flex justify-end pt-2">
                <Button variant="outline" size="sm" onClick={() => setShowApplyTemplateDialog(false)}>Close</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
