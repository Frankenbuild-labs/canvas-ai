...
export default function CampaignsCalendar({ campaigns }: CampaignsCalendarProps) {
  ...
  const campaignsByDate = useMemo(() => {
    ...
    campaignsArray.forEach((campaign: MarketingCampaign) => {
      const status = campaign.metadata?.status?.value || campaign.status
      
      // Use sent_at for sent campaigns, scheduled_for for scheduled, or send_date for others
      const campaignDate = status === "sent" || status === "Sent"
        ? (campaign.metadata?.sent_at || campaign.sent_at)
        : (campaign.scheduled_for || campaign.metadata?.send_date)
  ...
  router.push(`/email/campaigns/new?send_date=${formattedDate}`)
  }
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${day.isPast ? 'opacity-60' : 'hover:bg-muted'}
        ...
        <div className="mt-6 pt-6 border-t border-border">
          <div className="flex items-center gap-6 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">Past dates (campaigns shown)</span>
            </div>
            <div className="flex items-center gap-2">
              <Plus className="w-4 h-4 text-muted-foreground" />
              <span className="text-muted-foreground">Click future dates to create campaign</span>
            </div>
          </div>
        </div>
      </CardContent>
