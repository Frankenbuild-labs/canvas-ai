...
import { EmailContact, EmailList } from "@/lib/email-marketing/types";
import ConfirmationModal from "@/components/email-marketing/ConfirmationModal";
import EditContactModal from "@/components/email-marketing/EditContactModal";
import BulkActionsModal from "@/components/email-marketing/BulkActionsModal";
import ListManagementModal from "@/components/email-marketing/ListManagementModal";
import ContactsExportButton from "@/components/email-marketing/ContactsExportButton";
import Pagination from "@/components/email-marketing/ui/pagination";
...
export default function ContactsList({
  ...
  const handleListFilterChange = (value: string) => {
    // Use a sentinel for the UI select option since Radix Select Items cannot use an empty string value
    setListFilter(value === "none" ? "" : value);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await fetch("/api/email-marketing/contacts/bulk", {
        method: "DELETE",
  ...
  const handleListManagement = async (updates: {
    ...
    setBulkActionLoading(true);
    try {
      const response = await fetch("/api/email-marketing/contacts/bulk-lists", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  <div className="bg-card rounded-lg shadow-sm border border-border p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              // Show the sentinel value when listFilter is the empty string (meaning "No List")
              value={listFilter === "" ? "none" : listFilter || "all"}
        ...
        {searchTerm || statusFilter !== "all" || listFilter ? (
          <div className="mt-4 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {searchTerm && (
                <div className="inline-flex items-center gap-1 bg-accent text-accent-foreground text-xs px-2 py-1 rounded-full">
                  <span>Search: "{searchTerm}"</span>
                  <button
                    onClick={() => setSearchTerm("")}
                    className="hover:bg-accent/80 rounded-full p-0.5 text-accent-foreground"
                    aria-label="Clear search"
                    title="Clear search"
                  >
                    <X className="h-3 w-3" />
                        ...
                        contact.metadata.lists.map((list, index) => {
                          ...
                          return (
                            ...
                            >
                              <Badge
                                variant="outline"
                                className="text-xs bg-accent text-accent-foreground border border-border hover:bg-accent/80 cursor-pointer transition-colors"
                              >
                                {listName}
                              </Badge>
                            </button>
