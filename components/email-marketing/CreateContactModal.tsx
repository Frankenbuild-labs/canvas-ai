...
import { EmailList } from '@/lib/email-marketing/types'
...
export default function CreateContactModal({ children }: CreateContactModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/email-marketing/contacts', {
        method: 'POST',
      ...
      <DialogTrigger asChild>
        <>
          {children ? (
            ...
            <Button onClick={() => setIsOpen(true)} className="bg-teal-600 hover:bg-teal-700 text-white">
