...
export default function EditContactModal({ contact, children }: EditContactModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    status: contact.metadata.status.value as 'Active' | 'Unsubscribed' | 'Bounced',
    list_ids: [] as string[],
  ...
  // Set initial list membership when lists are loaded
  useEffect(() => {
    if (availableLists.length > 0 && contact.metadata.lists) {
      const currentListIds = contact.metadata.lists.map(list => 
        typeof list === 'string' ? list : list.id
      )
      setFormData(prev => ({ ...prev, list_ids: currentListIds }))
    }
  }, [availableLists, contact.metadata.lists])
