...
interface Message { id: string; from: string; to: string; subject?: string | null; created_at?: string; status?: string | null }

export default function OutboxPreviewPanel({ limit = 6 }: { limit?: number }) {
  const [items, setItems] = useState<Message[]>([])
  useEffect(() => { (async () => {
    try {
      const res = await fetch(`/api/email/outbox?limit=${limit}`, { cache: 'no-store' })
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-semibold">Outbox</h3>
        <Link href="/email/outbox" className="text-xs underline">Open</Link>
      </div>
