...
export type PegasusAction =
  ...
  | { type: "SET_TOOL_SETTINGS"; payload: PegasusState["toolSettings"] }
...
// Initial state
const initialState: PegasusState = {
  messages: [],
  isLoading: false,
...
// Create the context
const PegasusContext = createContext<{
  state: PegasusState;
  dispatch: React.Dispatch<PegasusAction>;
}>({
  state: initialState,
  dispatch: () => null,
});
...
// Reducer function
function pegasusReducer(state: PegasusState, action: PegasusAction): PegasusState {
  switch (action.type) {
    case "SET_MESSAGES":
...
export function PegasusProvider({ children }: { children: ReactNode }) {
  // Load initial state from cookies before creating the reducer
  const getInitialState = (): PegasusState => {
    // Start with the default initial state
    const defaultState = { ...initialState };

    // Try to load tool settings from cookies (shared with II-Agent)
    ...
    // Try to load selected model from cookies (shared with II-Agent)
  ...
  const [state, dispatch] = useReducer(pegasusReducer, getInitialState());

  return (
    <PegasusContext.Provider value={{ state, dispatch }}>
      {children}
...
export function usePegasusContext() {
  ...
  if (context === undefined) {
    throw new Error("usePegasusContext must be used within a PegasusProvider");
  }
  return context;
}
