// Apply CRM leads schema migration (supports Neon or local Postgres)
// Usage: node scripts/apply-crm-migration.mjs
...
async function main() {
  ...
  const sqlPath = path.resolve(process.cwd(), 'scripts', '004_create_crm_leads.sql');
  const ddl = await readFile(sqlPath, 'utf8');

  console.log('Applying CRM migration from', sqlPath);
  ...
  try {
    ...
    console.error('Failed to apply CRM migration:', err?.message || err);
    process.exit(1);
  }
}
